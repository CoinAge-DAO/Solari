...
*Setup instructions for a gitian build of Solari using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Solari
...
- [Building Solari](#building-solari)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-solari-host/README.md).
...
Clone the git repositories for solari and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/solari/solari
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the solari repository
...
To build Solari (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the solari repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/solari/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/solari/solari
...
For example:
```bash
URL=https://github.com/laanwj/solari.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit solari=${COMMIT} --url solari=${URL} ../solari/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit solari=${COMMIT} --url solari=${URL} ../solari/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit solari=${COMMIT} --url solari=${URL} ../solari/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solari-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solari-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/solari-osx-build.assert
```
...
[solari/gitian.sigs](https://github.com/solari/gitian.sigs/) repository, or if that's not possible create a pull
